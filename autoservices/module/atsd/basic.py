    ...
    try:
        ...
        for company_obj in company_obj_list:
            ...
            company_list.append({
                ...
                "licenses_used": company_obj.licenses_used,
                "max_license": company_obj.max_license,
                "session_config": company_obj.session_config,
                "assistant_config": company_obj.assistant_config,
                "badges_config": company_obj.badges_config,
            })
        return company_list
...
def getCompanyNameById(company_id):
    try:
        name = None
        company_obj = tbl_company.objects.get(id=company_id)
        name = company_obj.name
...
def registerCompany(name, group, twilio_account_sid, twilio_auth_token, twilio_phone_number,customerInfoAPI,fallbackForwardTo,
                    ...
                    unregisteredForwardTo, unregisteredEmailTo ,unregisteredSmsTo,unregisteredAPICall,licenses_used,max_license,session_config,assistant_config,badges_config):
    try:
        ...
        company_obj.licenses_used = licenses_used
        company_obj.max_license = max_license
        company_obj.session_config = session_config
        company_obj.assistant_config = assistant_config
        company_obj.badges_config = badges_config
        company_obj.save()
    ...
    try:
        ...
        for advisor_obj in advisor_obj_list:
            ...
            advisor_list.append({
                ...
                "alert_threshold": advisor_obj.alert_threshold,     # added  back
                "alert_phone_number": advisor_obj.alert_phone_number,
                "alert_email": advisor_obj.alert_email,
                "sms_allowed_from": advisor_obj.sms_allowed_from,
                "sms_allowed_to": advisor_obj.sms_allowed_to,
                "email_allowed_from": advisor_obj.email_allowed_from,
                "email_allowed_to": advisor_obj.email_allowed_to,
                "no_text_response_template":  advisor_obj.manager_response_template.replace("\n","slashn").replace("\\", "slashopposite"),
...
def registerAdvisor(advisor_info):
    try:
        ...
        advisor_obj.alert_threshold = advisor_info["alert_threshold"]    # added back
        advisor_obj.alert_phone_number = advisor_info["alert_phone_number"]
        advisor_obj.alert_email = advisor_info["alert_email"]
        advisor_obj.sms_allowed_from = advisor_info["sms_allowed_from"]
        advisor_obj.sms_allowed_to = advisor_info["sms_allowed_to"]
        advisor_obj.email_allowed_from = advisor_info["email_allowed_from"]
        advisor_obj.email_allowed_to = advisor_info["email_allowed_to"]
        advisor_obj.no_text_response_template = advisor_info["no_text_response_template"]
...
# User 2
# -----------------------------------------------------------------------------------
def getUser2List(filter_str=""):
    try:
        user_list = []
        user_obj_list = list(tbl_user2.objects.all())
        for user_obj in user_obj_list:
            flg = False
            if user_obj.user_name.find(filter_str) != -1:
                flg = True

            if flg == False:
                continue
            company_id = getCompanyNameById(user_obj.company_id)
            user_list.append({
                "user_name": user_obj.user_name,
                "phone_number": user_obj.phone_number,
                "email": user_obj.email,
                "voicemail_number": user_obj.voicemail_number,
                "company_id": company_id,
                "date_created": user_obj.date_created,
                "date_updated": user_obj.date_updated,
            })
        return user_list
