...
def registerUser2(user_name, phone_number, email, voicemail_number, company_id, date_updated, date_created):
    try:
        if len(list(tbl_user2.objects.filter(user_name=user_name))) > 0:
            return REG_EXISTING_EMAIL, None
            
        uid = random.randint(10000000, 99999999)
        while len(list(tbl_user.objects.filter(id=uid).first())) > 0:
            uid = random.randint(10000000, 99999999)
        ...
        user_obj.phone_number = phone_number
        user_obj.email = email
        user_obj.voicemail_number = voicemail_number
        user_obj.phone_number = phone_number
        user_obj.company_id = company_id
        user_obj.date_updated = date_updated
        user_obj.date_created = date_created
        user_obj.save()
