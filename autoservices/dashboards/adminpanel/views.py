...
def indexView(request, template_name="adminpanel/index.html"):
    try:
        ...
        user_list2 = atsd_basic.getUser2List()
...
def registerCompany(request):
    ...
    try:
        ...
        company_id = atsd_basic.registerCompany(data["name"], data["group"], data["twilio_account_sid"],
                                                ...
                                                data["unregisteredSmsTo"],data["unregisteredAPICall"],data["licenses_used"],
                                                data["max_license"],data["session_config"],data["session_config"],data["badges_config"])
        if company_id == None:
            return HttpResponse("error", status=HttpResponse500)
        if company_id == -1:
            return HttpResponse("existing_name", status=HttpResponse500)
...
def updateCompany(request):
    try:
        ...
        company_obj.licenses_used = data["licenses_used"]
        company_obj.max_license = data["max_license"]
        company_obj.session_config = data["session_config"]
        company_obj.assistant_config = data["assistant_config"]
        company_obj.badges_config = data["badges_config"]
        company_obj.save()
        return HttpResponse("success")
...
def registerUser2(request):
    try:
        ...
        data = json.loads(request.POST["param"])
        ret_code, user_obj = atsd_auth.registerUser2(data["user_name"], data["phone_number"],
                                                    data["email"], data["voicemail_number"],
                                                    data["company_id"],
        ...
        return HttpResponse(e, status=HttpResponse500)
...
def updateUser2(request):
    try:
        ...
        data = json.loads(request.POST["param"])
        user_obj2 = tbl_user2.objects.get(name=data["name"])
        user_obj2.user_name = data["user_name"]
        user_obj2.phone_number = data["phone_number"]
        user_obj2.email = data["email"]
        user_obj2.company_id = data["company_id"]
        user_obj2.voicemail_number = data["voicemail_number"]
        user_obj2.date_created = data["date_created"]
        user_obj2.date_updated = data["date_updated"]
        user_obj2.save()
        return HttpResponse("success")
...
def userView(request, template_name="adminpanel/user2.html"):
    try:
        email = request.session.get("email")
        user_obj = tbl_user.objects.get(email=email)
        if user_obj.is_superuser == 0:
        ...
        user_list = atsd_basic.getUser2List()
        total_user = len(user_list)
        total_page = math.ceil(total_user / 15)
        user_list = user_list[0:15]
...
def getPageUser2(request):
    try:
        ...
        filter_str = request.POST["filter_str"]
        cur_page = int(request.POST["cur_page"])
        user_list = atsd_basic.getUser2List(filter_str)
        total_user = len(user_list)
        total_page = math.ceil(total_user / 15)
        user_list = user2_list[15 * (cur_page - 1):15 * cur_page]
        data = {
            "total_page": total_page,
            "total_user": total_user,
            "user_list": user_list
...
def deleteUser2(request):
    try:
        ...
        id = int(request.POST["id"])
        user_obj = tbl_user2.objects.get(id=id)
        user_obj.delete()
